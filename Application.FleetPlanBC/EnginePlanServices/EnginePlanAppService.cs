...
namespace UniCloud.Application.FleetPlanBC.EnginePlanServices
{
    ...
    public class EnginePlanAppService : ContextBoundObject, IEnginePlanAppService
    {
        ...
        {
            var airlines = _airlinesRepository.GetAll().FirstOrDefault(p => p.IsCurrent); //获取航空公司
            var annual = _annualRepository.Get(dto.AnnualId); //获取计划年度

            //创建备发计划
            var newEnginePlan = EnginePlanFactory.CreateEnginePlan(dto.VersionNumber);
        ...
        public void ModifyEnginePlan(EnginePlanDTO dto)
        {
            ...
            //获取需要更新的对象
            var updateEnginePlan = _enginePlanRepository.Get(dto.Id);
            ...
            if (updateEnginePlan != null)
            {
                ...
                //更新接机行：
                var dtoEnginePlanHistories = dto.EnginePlanHistories;
        ...
        public void DeleteEnginePlan(EnginePlanDTO dto)
        {
            ...
            var delEnginePlan = _enginePlanRepository.Get(dto.Id);
            //获取需要删除的对象。
        ...
        {
            //获取
            var actionCategory = _actionCategoryRepository.Get(enginePlanHistoryDto.ActionCategoryId);
            var engineType = _engineTypeRepository.Get(enginePlanHistoryDto.EngineTypeId);
            var annual = _annualRepository.Get(enginePlanHistoryDto.PerformAnnualId);

            // 添加接机行
            var newEnginePlanHistory = enginePlan.AddNewEnginePlanHistory();
        ...
        private void UpdateEnginePlanHistory(EnginePlanHistoryDTO enginePlanHistoryDto, EnginePlanHistory enginePlanHistory)
        {
            //获取
            var actionCategory = _actionCategoryRepository.Get(enginePlanHistoryDto.ActionCategoryId);
            var engineType = _engineTypeRepository.Get(enginePlanHistoryDto.EngineTypeId);
