...
// 文件名：BasicConfigEntityConfiguration
// 版本：V1.0.0
//
// 修改者： 时间：
// 修改说明：
...
namespace UniCloud.Infrastructure.Data.PartBC.UnitOfWork.Mapping.Sql
{
    ...
    {
        public BasicConfigEntityConfiguration()
        {
            ToTable("BasicConfig", DbConfig.Schema);

            HasKey(p => p.Id);
            Property(p => p.Id).HasColumnName("ID").HasDatabaseGeneratedOption(DatabaseGeneratedOption.Identity);
            Property(p => p.BasicConfigGroupId).HasColumnName("BasicConfigGroupId");

            HasRequired(o => o.BasicConfigGroup).WithMany(t => t.BasicConfigs).HasForeignKey(o => o.BasicConfigGroupId);
        }
    }
}