...
namespace UniCloud.Presentation.FleetPlan.QueryAnalyse
{
    ...
    public class AgeDeployVm : ViewModelBase
    {
        ...
        /// <param name="e"></param>
        void AgeDeployGridViewCellEditEnded(object sender, GridViewCellEditEndedEventArgs e)
        {
            if (!e.Cell.Column.Header.ToString().Equals( "对应颜色",StringComparison.OrdinalIgnoreCase) && Convert.ToUInt32(e.NewData) != Convert.ToUInt32(e.OldData))
            {
                IsChanged = true;
            }
        ...
        {
            get { return _canAdd; }
            set { _canAdd = value; RaisePropertyChanged(() => CanAddBtn); }
        }
        #endregion
        ...
        private void OnRemove(object sender)
        {
            ...
            if (selectedAgeDeploy != null)
            {
                ...
                {
                    _ageDeployGridView.SelectedItem = AgeDeployCollection.Count() > selectIndex ? AgeDeployCollection[selectIndex] : AgeDeployCollection[AgeDeployCollection.Count() - 1];
                }
                //设置保存、放弃修改按钮的可用性
                IsChanged = true;
            }
        ...
        public bool CanRemoveBtn
        {
            ...
            set { _canRemove = value; RaisePropertyChanged(() => CanRemoveBtn); }
        }
        #endregion
        ...
        {
            get { return _canSave; }
            set { _canSave = value; RaisePropertyChanged(() => CanSaveBtn); }
        }
        #endregion
        ...
        public bool CanAbortBtn
        {
            ...
            set { _canAbort = value; RaisePropertyChanged(() => CanAbortBtn); }
        }
        #endregion
        #endregion
                    ...
                    if (xmlConfig != null)
                    {
                        ...
                        if (xelement != null)
                        {
                            foreach (var item in xelement.Descendants("Item"))
                            {
                                ...
                                if (ageColor != null)
                                {
                                    var firstOrDefault =ageColor.Descendants("Item").FirstOrDefault(p => p.Attribute("Name").Value.Equals(ageDeploy.Name,StringComparison.OrdinalIgnoreCase));
        ...
        protected void SaveAgeDeploXml()
        {
            ...
            foreach (AgeDeployClass agedeploy in AgeDeployCollection)
            {
                ...
                //供应商节点
                var itemNode = new XElement("Item", new XAttribute("Start", agedeploy.StartYear), new XAttribute("End", agedeploy.EndYear), name);
                ageDeploy.Add(itemNode);

                //颜色节点
                var childNode = new XElement("Item", new XAttribute("Name", name), new XAttribute("Color", agedeploy.Color));
                colorNode.Add(childNode);
            }
            if (XmlConfigs.FirstOrDefault(p => p.ConfigType.Equals( "机龄配置",StringComparison.OrdinalIgnoreCase)) != null)
            {
                var xmlConfigDataObject = XmlConfigs.FirstOrDefault(p => p.ConfigType.Equals( "机龄配置",StringComparison.OrdinalIgnoreCase));
            ...
            var xmlSetting = XmlSettings.FirstOrDefault(p => p.SettingType.Equals( "颜色配置",StringComparison.OrdinalIgnoreCase));
            if (xmlSetting != null)
            {
                var firstOrDefault = XElement.Parse(xmlSetting.SettingContent).Descendants("Type").FirstOrDefault(p => p.Attribute("TypeName").Value.Equals("机龄",StringComparison.OrdinalIgnoreCase));
                if (firstOrDefault != null)
                    firstOrDefault.Remove();
            }
            var settingDataObject = XmlSettings.FirstOrDefault(p => p.SettingType.Equals( "颜色配置",StringComparison.OrdinalIgnoreCase));
